 ...
 *
 * 이 파일에는 POST 를 보내는 길이 없다. 모든 요청은 guarded-get(GET 전용 관문)을 지나고, 토큰은 인증 모듈
 * (app-auth)의 공급자에게서 받을 뿐이다. 401 을 받으면 관문이 토큰을 한 번만 새로 받아 다시 시도한다.
 *
 ...
 *   - 한 번의 동기화에서 보내는 요청은 MAX_REQUESTS_PER_SYNC(1500)번까지. 넘으면 그 자리에서 멈추고 오류로 알린다
 *     (조용히 일부만 읽고 성공처럼 보이지 않게).
 */
...
export function createGitHubAppReader(options: AppReaderOptions): GitHubReader {
  const get = createGuardedGet({ tokens: options.tokens, fetch: options.fetch });
  ...
  const budget = () => {
    ...
    if (requests > maxRequests) {
      throw new GitHubReadError(`한 번의 Sync 요청 상한(${maxRequests}번)에 닿아 멈췄다. 저장소나 PR 이 너무 많다 — GITHUB_REPOS 로 좁힌다.`);
    }
  };
  ...
  return {
    ...
    async listRepositories() {
      requests = 0; // 동기화는 저장소 목록을 읽는 것으로 시작한다 — 요청 수를 새로 센다
      notes = [];
    ...
    async listPullRequests(repository) {
      ...
      const closed = asArray(await one(api(`${base}/pulls?state=closed&sort=updated&direction=desc&per_page=${CLOSED_PER_REPO}`)), "PR 목록");
      const out: PrSnapshot[] = [];
      for (const raw of [...open, ...closed]) {
        const pull = parsePull(raw);
        const checks = await one(api(`${base}/commits/${pull.headSha}/check-runs?per_page=100`));
        const reviews = await one(api(`${base}/pulls/${pull.number}/reviews?per_page=100`));
        out.push({
